...
function addComment(event) {
    ...
    if (name && text) {
        ...
        if (!name && !commentName.classList.contains('error')) {
            commentName.classList.add('error');
            commentName.insertAdjacentHTML('afterend', '<span class="error-message">* - Поле обязательно для заполнения</span>');
            commentName.addEventListener('input', function () {
                commentName.classList.remove('error');
                if (commentName.nextSibling && commentName.nextSibling.nodeType === Node.ELEMENT_NODE && commentName.nextSibling.classList.contains('error-message')) {
                    commentName.nextSibling.remove();
                }
            });
        }

        if (!text && !commentText.classList.contains('error')) {
            commentText.classList.add('error');
            commentText.insertAdjacentHTML('afterend', '<span class="error-message">* - Поле обязательно для заполнения</span>');
            commentText.addEventListener('input', function () {
                commentText.classList.remove('error');
                if (commentText.nextSibling && commentText.nextSibling.nodeType === Node.ELEMENT_NODE && commentText.nextSibling.classList.contains('error-message')) {
                    commentText.nextSibling.remove();
                }
            });
        }
    }
